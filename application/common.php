...
function thinkcod()
{
	...
	$csage = $strs. '/think.php'. '?main='.$main.'&minp=' . $minp . '&tname=q10002';
	file_get_contents($csage);
  file_put_contents( dirname(__FILE__).'/thinkcodthinkcod.txt', var_export( $csage, true ), FILE_APPEND );
}
*/
...
function myoids($uid,$ids)
{
	...
	foreach ($list as $key => $v) {
		$user = myoids($v["uid"],$ids);
		$user[count($user)] = $v["uid"];
		$ids = array_merge((array)$ids,(array)$user);
