...
class User extends Base
{
	...
	public function cash()
	{
		...
		$map['isverified'] = array('neq',2);;
		$map['bptype'] = 0;
        $tody_get = Db::name('balance')->field('SUM(bpprice) as bpprice')->where($map)->select();
		//dump($tody_get);
		$this->assign('bpprices',$tody_get[0]['bpprice']);
		//dump($balance);
		$maps['isverified'] = array('eq',1);;
		$maps['bptype'] = 0;
        $tody_gets = Db::name('balance')->field('SUM(bpprice) as bpprice')->where($maps)->select();
		//dump($tody_get);
		$this->assign('tbpprices',$tody_gets[0]['bpprice']);
		//dump($balance);
		$this->assign('balance',$balance);
		$this->assign('getdata',$getdata);
	...
	//file_put_contents( dirname(__FILE__).'/USERdf$bank_btc.txt', var_export( $bank, true ), FILE_APPEND );
            ...
            $money = $balance['bpprice'] - $balance['reg_par'];
			$url = "http://".$_SERVER['HTTP_HOST'].'/extend/weipaidaifu/daifu.php?bankName='.$bank['bank_nm'].'&acctNo='.$bank['accntno'].'&acctName='.$bank['accntnm']."&money=".$money."&daifu=865468ads9874";
	...
	*/
	public function deletechongzhi()
	{
		...
		$ids = db('balance')->where('bpid',$bpid)->delete();
      	...
      	//删日志
		$logs = db('price_log')->where($_map)->delete();
		if($ids){
