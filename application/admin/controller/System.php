...
class System extends Base
{
	...
	public function delslide()
	{
        ...
        $del = Db::name('userinfo')->where('uid', $id)->delete();
        if($del){
	...
	*/
	...
	public function rateInfo(){
		$rid = input('param.rid');
		$rate = Db::name('rate')->where('rid',$rid)->find();
		$this->assign($rate);
		return $this->fetch();
	}
	/**
	...
	public function rechargeRate()
	{
		...
		$list = Db::name('rate')->order('rid desc')->select();
		$this->assign('list',$list);
		return $this->fetch();
	}
	/**
