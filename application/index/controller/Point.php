...
class Point extends Base
{
	...
	public function point()
	{
		...
		->where('pi.isdelete=0 and pi.trade_type = 2')->order('pi.proorder asc')->limit(3)->select();

        /*$proname = Db::name('productinfo')->alias('pi')->field('pi.pid,pi.ptitle')
       		->where('pi.isdelete=0 and pi.trade_type = 2')->order('pi.proorder asc')->select();*/
		...
		if(!$pid){
			...
			if(sizeof($proname1) == 0){
				$this->redirect('/index/point/point/');
			}
			$pid = $proname1[0]['pid'];
		}
