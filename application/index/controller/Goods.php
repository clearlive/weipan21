...
class Goods extends Base
{
	...
	public function goods()
	{
		...
		$proname1 =Db::name('productinfo')->alias('pi')->field('pi.pid,pi.procode,pi.ptitle,pi.jianjie,pd.Price,pd.UpdateTime,pd.Low,pd.High')
		->join('__PRODUCTDATA__ pd','pd.pid=pi.pid')
		->where('pi.isdelete=0 and pi.trade_type = 1')->order('pi.proorder asc')->limit(3)->select();

        /*$proname = Db::name('productinfo')->alias('pi')->field('pi.pid,pi.ptitle')
       		->where('pi.isdelete=0 and pi.trade_type = 1')->order('pi.proorder asc')->select();*/
       		
		$pid = input('param.pid');
        //file_put_contents( dirname(__FILE__).'/QTB_________URL.txt', var_export($pid, true ), FILE_APPEND );
		if(!$pid){
			if(sizeof($proname1) == 0){
				$this->redirect('/index/goods/goods/');
			}
			//如果不传入pid，那么就自动获取第一个
			$pid = $proname1[0]['pid'];
		}
		...
		if(!$pro){
			$this->redirect('/index/goods/goods/'.$pid);
		}
		...
		$proclaima = getconf('proclaima');;
      	$this->assign('proclaima',$proclaima);
      
		
		//dump($pay_choose);exit;
		$this->assign('pro',$pro);
		$this->assign('proname1',$proname1);
