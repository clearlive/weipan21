...
class Order extends Base
{
	...
	public function addorder()
	{
		...
		*/
		...
		//用户信息
		$user = Db::name('userinfo')->field('ustatus,usermoney,userdollor,uid')->where('uid',$data['uid'])->find();
		//验证用户是否被冻结
		if($user['ustatus'] != 0){
		...
		$web_poundage_USDT = round($web_poundage*$bl[0]['gamecoin']/$bl[0]['rmbcoin'],2);
		
		//验证余额是否够
		/*if($user['usermoney'] < $data['order_price'] + $web_poundage){
			return WPreturn('您得余额不足，请充值！',-1);
		}*/
		if($user['userdollor'] < $data['order_price'] + $web_poundage_USDT){
			return WPreturn('您得余额不足，请充值！',-1);
		}
		//验证金额 20 ~ 5000
		...
		$adddata['trade_type'] = $data['trade_type'];
		...
		if($data['trade_type'] == 1){
			$adddata['selltime']=$adddata['buytime']+$adddata['endprofit'];//时间盘
		}
		...
		$adddata['sx_fee']=round($adddata['fee']*$web_poundage_USDT/100 ,2);
        $allfee = $adddata['fee'] + $adddata['sx_fee'];
        //会员建仓后金额
        //$adddata['commission'] = $user['usermoney'] - $allfee;
        $adddata['commission'] = $user['userdollor'] - $allfee;
        //订单号
        ...
        if($ids){
        	...
        	$editmoney = Db::name('userinfo')->where('uid',$data['uid'])->setDec('userdollor',$u_fee);
        	//减人民币
        	Db::name('userinfo')->where('uid',$data['uid'])->setDec('usermoney',round($u_fee*$bl[0]['rmbcoin']/$bl[0]['gamecoin'],2));
