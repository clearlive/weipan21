...
class User extends Base
{
	...
	public function index()
	{
		...
		/*$detail= Db::name('order')->where(array('uid'=>$uid,'ostaus'=>0))->select();*/
		$orders = Db::name('order')->field("count(*) orderCount")->where(array('uid'=>$uid,'ostaus'=>0))->select();
		//出金------------------------------------------
		//银行卡
        ...
        //充值方式
        $payment = db('payment')->where(array('isdelete'=>0,'is_use'=>1))->order('pay_order asc ')->select();
        if($payment){
        	$arr2 = $arr = $arr1 = array();
	...
	public function addNewBalance(){
		...
		if(!$post['pay_type'] || !$post['bpprice']){
   		...
   		$nowtime = time();
   		$bl = Db::name('rate')->where('rid=1')->select();
   		$type = 2;//1 支付宝 2 QQ支付 3 微信支付
   		if($post['pay_type'] == 'alipay'){
   			$type = 1;
   		}else if($post['pay_type'] == 'wxpay'){
   			$type = 3;
   		}
   		//插入充值数据
   		$data['bptype'] = 3;
   		$data['bptime'] = $nowtime;
   		$money = $post['bpprice'];
   		$data['bpprice'] = round($money * $bl[0]['gamecoin']/$bl[0]['rmbcoin'],2);
   		$data['remarks'] = '会员充值';
   		$data['uid'] = $uid;
   		$data['isverified'] = 0;
   		$data['btime'] = $nowtime;
   		...
   		//$data['bpbalancedollor'] = $user['userdollor']+$data['bpprice'];
   		$data['bpbalancedollor'] = $user['userdollor'];
      	include($_SERVER["DOCUMENT_ROOT"].'/mcbpay/mcbconfig.php');
      	$ids = db('balance')->insertGetId($data);    
   		if($ids){
   			$return_url = $_SERVER["HTTP_HOST"].'/index/mcbpay/return_url?bpid='.$ids.'&money='.$money.'&uid='.$uid;
   			$return_url = urlencode($return_url);
   			$url = CODEPAY_URL.'?id='.CODEPAY_APPID.'&token='.CODEPAY_APPTOKEN.'&pay_id='.$this->user['utel'].'&price='.$money.'&type='.$type.'&return_url='.$return_url;
   			//$url .= $url.'&price='.$money.'&type='.$type.'&notify_url='.$notify_url;
   			return $url;
   		}
	}
